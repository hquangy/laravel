		...
		<div class="col-sm-12">
			<div class="box box-primary">
				<div class="box-header with-border">
					<h3 class="box-title">Trang quản lý Blog</h3>
					...
					<table id="blog-table" class="table table-striped table-hover">
					    ...
					    <tbody>
					        ...
					        <tr>
					            <td>{{ $blog->id}}</td>
					            <td>{{ $blog->title }}</td>
					            <td>{!! $blog->visibility ? '<span class="label label-success">Hiện</span>' : '<span class="label label-danger">Ẩn</span>' !!}</td>
					            <td>{{ $blog->boost }}</td>
					            <td>{{ word_limit($blog->short_description,10) }}</td>
					            <td>
					            	<a href="{{ route('backend.blog.edit',['id'=>$blog->id])}}" class="btn btn-info btn-xs">
